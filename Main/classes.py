...
import Features.features as feature
...
class CharClass:
    ...
    name = "Default"
    level = 1
    hit_dice_faces = dice.D10
    subclass_select_level = 3
    weapon_proficiencies = ()
    proficiencies_text = ()
    multiclass_weapon_proficiencies = ()
    multiclass_proficiencies_text = ()
    ...
    def __init__(self, level, owner=None, subclass=None, feature_choices=[],
                 **params):
        self._multiclass_proficiencies_text = None
        self.level = level
        self.owner = owner
        ...
        for i in range(1, 21):
            ...
            for f in cls.features_by_level[i]:
                if issubclass(f, feature.FeatureSelector):
                    fs.append(f(owner=self.owner,
                                feature_choices=feature_choices))
                elif issubclass(f, feature.Feature):
                    fs.append(f(owner=self.owner))
    ...
    def apply_subclass(self, feature_choices=[]):
        ...
        self._multiclass_proficiencies_text += tuple(subcls.proficiencies_text)
